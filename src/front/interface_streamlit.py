...
def build_sidebar() -> dict[str, Any]:
    with ui_helper.card_block(st.sidebar) as sidebar:
        ...
        ollama_url = sidebar.text_input(
            "Ollama URL", value=DEFAULT_OLLAMA_URL, help="URL du serveur Ollama local"
        )
        ...
        vote_n = sidebar.slider(
            "Nombre d'appels (vote)", 1, 7, 1, help=">1 = vote majoritaire pour plus de robustesse"
        )
        ...
        threshold = sidebar.slider(
            "Seuil de confiance pour basculer en 'Autre'", 0.0, 1.0, 0.35, 0.01
        )

        sidebar.markdown("### 🧩 Instructions (optionnel)")
        extra_instructions = sidebar.text_area(
            "Contexte/contraintes métier",
            placeholder="Ex: Si 'TVA' ou 'devis' -> Facture prioritaire...",
        )
...
def choose_categories_ui(df: pd.DataFrame | None, parent_right) -> list[str]:
    with ui_helper.card_block(parent_right) as right_card:
        ...
        # Aperçu propre
        categories_preview = (
            pd.DataFrame({"Categorie": labels}) if labels else pd.DataFrame(columns=["Categorie"])
        )
        ...
        right_card.markdown(
            "<small class='help'>Astuce: garde 3–8 catégories au début. Tu peux ajouter 'Autre' toi-même ou cocher l’option.</small>",
...
def choose_method_ui(df: pd.DataFrame | None) -> str:
...
def choose_sampling_ui(df: pd.DataFrame) -> tuple[int, bool]:
    ...
    with ui_helper.card_block() as params_card:
        ...
        do_full = params_card.toggle(
            "Exécuter sur TOUT le dataset (sinon: échantillon)", value=False
        )
...
def configure_run_params_ui(df: pd.DataFrame, labels: list[str]) -> tuple[str, int, bool, bool]:
    ...
    with ui_helper.card_block() as params_card:
        ...
        run = params_card.button(
            "🚀 Lancer le classement (LLM)", type="primary", use_container_width=True
        )
...
def run_llm_pipeline(
    ...
    with ui_helper.card_block() as results_card:
        ...
        results_card.dataframe(
            pd.DataFrame({"label": counts.index, "count": counts.values}), use_container_width=True
        )
...
def run_smoke_tests() -> dict[str, Any]:
    ...
    # 1) Constants are set
    assert isinstance(DEFAULT_OLLAMA_URL, str) and DEFAULT_OLLAMA_URL.startswith(
        "http"
    ), "DEFAULT_OLLAMA_URL invalid"
...
def main() -> None:
    ...
    if df is not None and len(labels) > 0:
        ...
        if method == "LLM":
            ...
            emb_section.embeddings_section_ui(
                df=df,
                labels=labels,
                do_full=do_full,
                sample_n=sample_n,
                default_text_col=text_col if "text_col" in locals() else None,
            )
