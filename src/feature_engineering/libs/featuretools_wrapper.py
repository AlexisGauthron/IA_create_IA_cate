...
class AutoFeatureEngineer:
    ...
    def create_entity(self):
        ...
        """
        if self.verbose:
            print(f"[INFO] Création de l'entité '{self.target_entity}' avec index '{self.index}' et time_index '{self.time_index}'")
    ...
    def run_dfs(self):
        ...
        self.features_df, self.feature_defs = ft.dfs(
            ...
            features_only=False,
            ignore_columns={self.target_entity: self.ignore_columns} if self.ignore_columns else None
        )
        if self.verbose:
