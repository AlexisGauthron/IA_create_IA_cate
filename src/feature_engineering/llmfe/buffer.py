...
import dataclasses
import time
from typing import Any, Tuple, Mapping, Optional, TYPE_CHECKING
...
from src.feature_engineering.llmfe.utils import serialize
from src.feature_engineering.llmfe.utils import is_categorical
    ...
    def _generate_prompt(
        ...
        if self._path_config is not None:
            try:
                ...
                logging.warning(f"Prompts {prompt_type} non trouvés via path_config, utilisation du fallback")
            ...
            if is_cat_sel[cidx] == True:
                ...
                try:
                    ...
                    if pd.isna(min_val) or pd.isna(max_val):
                        ...
                        feature_name_list.append(f"- {cname}: {desc} (mixed type with values [{clist_str}])")
                    else:
                        desc = self._meta_data.get(cname, cname.replace('_', ' '))
